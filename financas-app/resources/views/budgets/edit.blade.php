...
<div class="max-w-screen-2xl mx-auto py-6 animate-slide-in-up">
    ...
    <div class="card-modern animate-fade-in-scale mb-8">
        <div class="flex items-center justify-between p-8 border-b border-gray-200 dark:border-gray-700">
            <div class="flex items-center space-x-4">
                <div class="w-16 h-16 bg-gradient-to-br from-blue-500 to-indigo-600 rounded-xl flex items-center justify-center animate-float">
                    <i class="fas fa-edit text-white text-2xl"></i>
                </div>
                <div>
                    <h1 class="text-3xl font-bold text-gray-900 dark:text-white mb-2">
                        <i class="fas fa-edit text-blue-500 mr-3"></i>{{ __('Editar Orçamento') }}
                    </h1>
                    <p class="text-gray-600 dark:text-gray-300">Atualize as informações do orçamento "{{ $budget->name }}"</p>
                </div>
            </div>
            <div class="flex space-x-3">
                <a href="{{ route('budgets.show', $budget) }}" class="btn-gradient-secondary animate-glow transform hover:scale-105 transition-all duration-300">
    ...
    <div class="card-modern animate-fade-in-scale">
        <div class="p-8">
            ...
            <div class="bg-gradient-to-r from-blue-50 to-indigo-50 dark:from-blue-900/20 dark:to-indigo-900/20 border border-blue-200 dark:border-blue-700 rounded-lg p-6 mb-8">
                <div class="flex items-center justify-between">
                    <div class="flex items-center space-x-4">
                        <div class="w-14 h-14 rounded-lg bg-gradient-to-r" style="background: linear-gradient(135deg, {{ $budget->category->color }}20, {{ $budget->category->color }}40); border: 2px solid {{ $budget->category->color }};">
                            @if($budget->category->icon)
                                <i class="{{ $budget->category->icon }} text-xl flex items-center justify-center h-full" style="color: {{ $budget->category->color }};"></i>
                            @else
                                <i class="fas fa-chart-pie text-xl flex items-center justify-center h-full" style="color: {{ $budget->category->color }};"></i>
                            @endif
                        </div>
                        <div>
                            <h3 class="text-xl font-bold text-blue-800 dark:text-blue-200">{{ $budget->name }}</h3>
                            <p class="text-blue-600 dark:text-blue-300">{{ $budget->category->name }}</p>
            ...
            <form action="{{ route('budgets.update', $budget) }}" method="POST" class="space-y-6">
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
                    ...
                    <div class="space-y-6">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" 
                                   ...
                                   class="form-control w-full px-4 py-3 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-300 @error('name') border-red-500 @enderror"
                        ...
                        <div class="form-group">
                            ...
                            <select id="category_id" 
                                    name="category_id" 
                                    class="form-control w-full px-4 py-3 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-300 @error('category_id') border-red-500 @enderror"
                                    required>
                        ...
                        <div class="form-group">
                            ...
                            <div class="relative">
                                ...
                                <input type="number" 
                                       ...
                                       class="form-control w-full pl-12 pr-4 py-3 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-300 @error('amount') border-red-500 @enderror"
                                       placeholder="0,00"
                                       step="0.01"
                                       min="0.01"
                                       required>
                    ...
                    <div class="space-y-6">
                        ...
                        <div class="form-group">
                            ...
                            <select id="period" 
                                    name="period" 
                                    class="form-control w-full px-4 py-3 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-300 @error('period') border-red-500 @enderror"
                                    required>
                                <option value="">Selecione o período</option>
                                <option value="weekly" {{ old('period', $budget->period) == 'weekly' ? 'selected' : '' }}>Semanal</option>
                                <option value="monthly" {{ old('period', $budget->period) == 'monthly' ? 'selected' : '' }}>Mensal</option>
                                <option value="quarterly" {{ old('period', $budget->period) == 'quarterly' ? 'selected' : '' }}>Trimestral</option>
                        ...
                        <div class="form-group">
                            ...
                            <input type="date" 
                                   ...
                                   class="form-control w-full px-4 py-3 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-300 @error('start_date') border-red-500 @enderror"
                                   required>
                            @error('start_date')
                                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                            @enderror
                        ...
                        <div class="form-group">
                            ...
                            <div class="relative">
                                <input type="number" 
                                       ...
                                       value="{{ old('alert_percentage', $budget->alert_percentage) }}" 
                                       class="form-control w-full px-4 py-3 pr-12 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-300 @error('alert_percentage') border-red-500 @enderror"
                                       min="50"
                                       max="100"
                                       step="5"
                                       required>
                ...
                <div class="form-group">
                    ...
                    <textarea id="description" 
                              name="description" 
                              rows="4" 
                              class="form-control w-full px-4 py-3 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-300 @error('description') border-red-500 @enderror"
                              placeholder="Adicione uma descrição para este orçamento...">{{ old('description', $budget->description) }}</textarea>
                    @error('description')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="form-group">
                    <label class="flex items-center space-x-3">
                        ...
                        <span class="text-sm font-medium text-gray-700 dark:text-gray-300">
                            <i class="fas fa-toggle-on text-green-500 mr-2"></i>Orçamento Ativo
                        </span>
                    </label>
                    <p class="text-gray-500 text-sm mt-1 ml-8">
                        <i class="fas fa-info-circle mr-1"></i>
                        Desmarque para pausar o orçamento temporariamente
                    </p>
                </div>
                ...
                <div class="flex items-center justify-between pt-6 border-t border-gray-200 dark:border-gray-700">
                    <div class="flex space-x-3">
                        ...
                        <form action="{{ route('budgets.destroy', $budget) }}" method="POST" class="inline" 
                            ...
                            @method('DELETE')
                            <button type="submit" 
                                    class="px-6 py-3 bg-gradient-to-r from-red-500 to-red-600 text-white rounded-lg hover:from-red-600 hover:to-red-700 transform hover:scale-105 transition-all duration-300 shadow-lg hover:shadow-xl">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Aviso sobre mudanças importantes
    const categorySelect = document.getElementById('category_id');
    const originalCategory = '{{ $budget->category_id }}';
    
    categorySelect.addEventListener('change', function() {
        if (this.value !== originalCategory && this.value !== '') {
            const warning = document.createElement('div');
            warning.className = 'bg-yellow-100 border border-yellow-400 text-yellow-700 px-4 py-3 rounded-lg mt-2 text-sm animate-slide-in-up';
            warning.innerHTML = '<i class="fas fa-exclamation-triangle mr-2"></i>Alterar a categoria irá recalcular os gastos deste orçamento.';
            
            // Remove avisos anteriores
            const existingWarning = this.parentNode.querySelector('.bg-yellow-100');
            if (existingWarning) {
